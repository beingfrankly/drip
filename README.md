...
Note: `drip fetch` remembers what it's already written to a digest, per source — an item that appeared in an *earlier* written digest won't be included again (an item is recorded as seen only when it's actually written to a digest, never on a `--dry-run`), so each item shows up in exactly one digest. If a fetch turns up nothing new, `drip` says so and leaves the day's note unchanged.
