...
- **The digest note is one-per-day, named `<YYYY-MM-DD> - Daily digest.md`** (local ISO date, no time, no topic/source label). Because the name has no time, a second `drip fetch` on the same calendar day **overwrites** that day's note — and since dedup means the re-run only carries items not already seen, it would replace the note with *fewer* items. The daily cron runs once per day, so this is normally a non-issue; just avoid ad-hoc same-day re-runs unless you mean to replace the note.
